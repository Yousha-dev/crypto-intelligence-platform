...
from myapp.serializers.admin_serializers import PaymentSerializer, SubscriptionPlanSerializer, SubscriptionSerializer
from myapp.models import (
    PAYMENT_METHOD_CHOICES, SUBSCRIPTION_STATUS_CHOICES, Subscriptionplans, Users, 
    PAYMENT_STATUS_CHOICES, Subscriptions, Payments
) 
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                'currency': openapi.Schema(type=openapi.TYPE_STRING, description="Currency code", default="usd"),
            },
            required=['amount']
        ),
        responses={
            200: openapi.Schema(
                type=openapi.TYPE_OBJECT,
                properties={
                    'client_secret': openapi.Schema(type=openapi.TYPE_STRING, description="Stripe client secret")
                }
            ),
            400: "Bad Request",
...
class PaymentServiceStatusAPI(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            'message': 'Payment services are operational' if stripe_enabled else 'Payment services are temporarily disabled for maintenance'
        }, status=status.HTTP_200_OK)
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                'role': openapi.Schema(type=openapi.TYPE_STRING, description="User role", default='User'),
                ...
                'frequency': openapi.Schema(type=openapi.TYPE_STRING, description="Billing frequency"),
                'subscriptionStatus': openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=[choice[0] for choice in SUBSCRIPTION_STATUS_CHOICES],
                    description="Subscription status"
                ),
                'Autorenew': openapi.Schema(type=openapi.TYPE_INTEGER, description="Auto-renewal flag"),
                'paymentAmount': openapi.Schema(type=openapi.TYPE_NUMBER, description="Payment amount"),
                'paymentMethod': openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=[choice[0] for choice in PAYMENT_METHOD_CHOICES],
                    description="Payment method"
                ),
                'ReferenceNumber': openapi.Schema(type=openapi.TYPE_STRING, description="Payment reference number"),
                'paymentStatus': openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=[choice[0] for choice in PAYMENT_STATUS_CHOICES],
                    description="Payment status"
                ),
                'PaymentResponse': openapi.Schema(type=openapi.TYPE_STRING, description="Payment response")
            },
            required=['fullname', 'email', 'password', 'subscriptionplanid', 'frequency', 'paymentAmount', 'paymentMethod']
        ),
        responses={
    ...
    def post(self, request):
        try:
            ...
            user_data = {
                ...
                'logo': data.get('logo'),
                'trading_experience': data.get('trading_experience'),
            ...
            subscription_data = {
                'userid': user.userid,
                'subscriptionplanid': data.get('subscriptionplanid'),
            ...
            payment_data = {
                ...
                'paymentdate': current_time.date(),
                'paymentmethod': data.get('paymentMethod', 'CreditCard'),
...
class RegisterUserAPI(APIView):
    ...
    @swagger_auto_schema(
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'fullname': openapi.Schema(type=openapi.TYPE_STRING, description="User's full name"),
                'email': openapi.Schema(type=openapi.TYPE_STRING, description="User email"),
                'password': openapi.Schema(type=openapi.TYPE_STRING, description="User password"),
                'role': openapi.Schema(
                    type=openapi.TYPE_STRING,
                ...
                'zipcode': openapi.Schema(type=openapi.TYPE_STRING, description="Zip code"),
                'country': openapi.Schema(type=openapi.TYPE_STRING, description="Country"),
                'logo': openapi.Schema(type=openapi.TYPE_STRING, description="Base64 encoded logo"),
                'trading_experience': openapi.Schema(
                    type=openapi.TYPE_STRING,
                    enum=['beginner', 'intermediate', 'advanced'],
                    description="Trading experience level"
    ...
    def post(self, request):
        ...
        if role not in ['Admin', 'User']:
            return Response({"error": "Invalid role. Allowed roles are 'Admin' and 'User'."},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        user_data = {
            ...
            "trading_experience": request.data.get("trading_experience"),
...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        try:
            # Fetch the user
            user = Users.objects.filter(email=email, isactive=1, isdeleted=0).first()
            if not user:
            ...
            userData = {
                ...
                "avatar": f"{settings.MEDIA_URL}{user.logo_path}" if user.logo_path else "https://duedoom.com/src/assets/images/avatars/avatar-1.png",
...
class RequestPasswordResetAPI(APIView):
    ...
    def post(self, request):
        try:
            ...
            html_message = get_password_reset_template(reset_url, user.fullname)
...
class ResetPasswordAPI(APIView):
    ...
    def post(self, request):
        try:
            ...
            except (SignatureExpired, BadSignature, Users.DoesNotExist):
                return Response({
...
class ChangeUserPasswordAPI(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            if not check_password(old_password, user.passwordhash):
                return Response(
...
class DeactivateUserAPI(APIView):
    ...
    def post(self, request):
        user_id = request.data.get('user_id')
        try:
            user = Users.objects.get(userid=user_id, isactive=1)
            user.isactive = 0
            user.updatedat = timezone.now()
            user.save()
...
class SendEmailAPI(APIView):
    ...
    def post(self, request):
        try:
            ...
            to_email = request.data.get('to_email', 'support@duedoom.com')
    ...
    <body style="margin: 0; padding: 0; font-family: Arial, sans-serif; line-height: 1.6; background-color: #f5f5f5;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="text-align: center; padding-top: 20px; color: #666666; font-size: 12px;">
                <p>This is an automated message from your Crypto Trading Platform. Please do not reply.</p>
...
def get_generic_email_template(message: str, title: Optional[str] = None, has_attachment: bool = False) -> str:
