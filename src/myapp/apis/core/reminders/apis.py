...
from myapp.models import Reminders, Users
...
class DeleteReminderAPI(APIView):
    ...
    def delete(self, request, reminder_id):
        ...
        try:
            ...
            reminder.isdeleted = 1
            reminder.updatedby = user_id
            reminder.updatedat = timezone.now()
            reminder.save()
            return Response(
    ...
    def post(self, request, reminder_id):
        try:
            ...
            reminder = Reminders.objects.filter(
                ...
                isactive=1,
                isdeleted=0
            ).select_related('extensionsubscriptionid').first()
            ...
            # Get recipient list
            recipient_list = list(Users.objects.filter(
            ...
            if reminder.extensionsubscriptionid:
                subscription = reminder.extensionsubscriptionid
                subject = f"Reminder: {subscription.servicename} Subscription"
                html_message = get_reminder_template(
                    subscription_name=subscription.servicename,
            ...
            return Response(
                {"error": f"Error sending reminder email: {str(e)}"},
    ...
    def get(self, request):
        try:
            ...
            due_reminders = Reminders.objects.filter(
                ...
                isactive=1,
                isdeleted=0
            ).select_related('extensionsubscriptionid')
            ...
            for reminder in due_reminders:
                try:
                    ...
                    if reminder.extensionsubscriptionid:
                        subscription = reminder.extensionsubscriptionid
                        subject = f"Reminder: {subscription.servicename} Subscription (30 minutes notice)"
                        html_message = get_reminder_template(
                            subscription_name=subscription.servicename,
                            reminder_note=reminder.note,
                            end_date=subscription.enddate,
                    ...
                    # Mark reminder as inactive after sending
                    reminder.isactive = 0
                    reminder.updatedby = None # Automated task
                    reminder.updatedat = timezone.now()
                    reminder.save()
                    sent_count += 1

                except Exception as e:
                    print(f"Error processing reminder {reminder.reminderid}: {str(e)}")
                    continue
