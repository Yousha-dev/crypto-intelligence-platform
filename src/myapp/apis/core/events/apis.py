...
from myapp.models import Events, Users
    ...
    def get(self, request):
        ...
        if start_date:
            try:
                ...
                filters['startdate__gte'] = start_date
            except ValueError:
...
class UpdateEventAPI(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=EventSerializer,
        responses={200: "Event updated successfully.", 404: "Event not found.", 400: "Validation errors."},
    )
    def put(self, request, event_id):
        ...
        try:
            event = Events.objects.get(pk=event_id, userid=user_id, isdeleted=0)
...
class DeleteEventAPI(APIView):
    ...
    def delete(self, request, event_id):
        ...
        try:
            event = Events.objects.get(pk=event_id, userid=user_id, isdeleted=False)
            event.isdeleted = 1
...
class AutoSendActionEmailEventAPI(APIView):
    ...
    def get(self, request):
        try:
            ...
            action_events = Events.objects.filter(
                type='Action',
                enddate__gte=today,
                startdate__lte=one_day_future,
                isactive=1,
                isdeleted=0
            )
            ...
            for event in action_events:
                ...
                for event_date in event_dates:
                    if self._should_send_email(event_date, event.starttime):
                        # Prepare recipient lists
                        to_recipients = []
                        cc_recipients = []
                        
                        if event.emailto:
                            to_recipients = [email.strip() for email in event.emailto.split(',')]
                            
                        if event.emailcc:
                            cc_recipients = [email.strip() for email in event.emailcc.split(',')]
                        ...
                        if to_recipients:
                            ...
                            try:
                                user = Users.objects.get(userid=event.userid.userid)
                                if user.useusersmtp == 1:
                                    try:
                                        # Create connection with user SMTP settings
                                        connection = get_connection(
                                            host=user.smtphost,
                                            port=user.smtpport,
                                            username=user.smtphostuser,
                                            password=user.smtphostpassword,
                                            use_tls=user.smtpusetls,
                                            fail_silently=False
                                        )
                                        
                                        # Test the connection
                                        connection.open()
                                        logger.info(f"User SMTP connection successful for event {event.eventid}")
                                        from_email = user.smtphostuser  # Use user's email as sender
                                        
                                    except Exception as e:
                                        logger.error(f"Failed to connect using user SMTP settings: {str(e)}")
                                ...
                                logger.error(f"User {event.userid.userid} not found")
                            
                            # Send email with appropriate connection and from_email
                            email_sent = email_helper.send_email(
                                subject=event.emailsubject,
                                message=event.emailbody,  # Plain text fallback
            ...
            logger.error(f"Error in AutoSendActionEmailEventAPI: {str(e)}")
            return Response(
                {"error": str(e)},
    ...
    def _create_notification(self, user_id, event_id, event_title, recipients):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                logger.error(f"Event notification validation error: {serializer.errors}")
                
        except Exception as e:
            logger.error(f"Error creating event notification: {str(e)}")


def get_action_event_email_template(event_title, event_description, event_date, event_time, event_location):
    """
...
class AutoSendReminderEmailEventAPI(APIView):
    ...
    def _create_notification(self, user_id, event_id, event_title, reminder_type):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                logger.error(f"Event reminder notification validation error: {serializer.errors}")
                
        except Exception as e:
            logger.error(f"Error creating event reminder notification: {str(e)}")
            ...
            200: openapi.Response(
                ...
                schema=openapi.Schema(
                    ...
                    properties={
                        ...
                        'hour_reminders_sent': openapi.Schema(type=openapi.TYPE_INTEGER)
                    }
                )
            ),
            500: openapi.Response(
    ...
    def get(self, request):
        try:
            ...
            one_day_future = today + timedelta(days=1)
            reminder_events = Events.objects.filter(
                type='Reminder',
                enddate__gte=today,
                startdate__lte=one_day_future,
                isactive=1,
                ...
                for event_date in event_dates:
                    ...
                    if should_send:
                        ...
                        if to_recipients:
                            ...
                            reminder_period = "1 day" if reminder_type == 'day' else "1 hour"
            ...
            logger.error(f"Error in AutoSendReminderEmailEventAPI: {str(e)}")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
def get_reminder_event_email_template(event_title, event_description, event_date, event_time, event_location, reminder_period):
    """
